...
def load_template(name):
    """Load template from file, or return embedded fallback."""
...
def analyze_sentiment(text):
    ...
    negative_words = ["problem", "issue", "bad", "terrible", "awful", "disappointed", "unhappy",
                     ...
                     "refund", "cancel", "urgent", "critical", "broken", "bug"]
...
def fetch_emails_from_imap(account_id, email_addr, app_password, imap_server, folder="INBOX", limit=200):
    ...
    try:
        ...
        message_ids = message_ids[-limit:]  # Take last N
            ...
            try:
                ...
                try:
                    c.execute("""
                        ...
                        (account_id, message_id, subject, sender, sender_email, recipient,
                         date, body, snippet, folder, sentiment, sentiment_score,
                         sentiment_positive, sentiment_negative, word_count)
                        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """, (account_id, msg_id, subject, sender, sender_email, recipient,
                          date_iso, body, snippet, folder, sentiment, score, pos, neg, word_count))
                    conn.commit()
                    if c.rowcount > 0:
                        synced += 1
                except Exception:
...
def get_stats():
    ...
    top_senders = conn.execute(f"""
        SELECT sender_email, COUNT(*) as cnt FROM emails {clause}
        GROUP BY sender_email ORDER BY cnt DESC LIMIT 10
    """, params).fetchall()
    
    # Daily trend (last 30 days)
    trend_clause = f"WHERE date >= datetime('now', '-30 days')" + (" AND " + " AND ".join(where) if where else "")
