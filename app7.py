...
def fetch_emails_from_imap(account_id, email_addr, app_password, imap_server, folder="INBOX", limit=None):
    conn = get_db()
    synced = 0
    errors = []
    try:
...
def get_stats():
    ...
    top_senders = conn.execute(f"""
        ...
        GROUP BY sender_email ORDER BY cnt DESC LIMIT 50
    """, params).fetchall()
    ...
    top_subjects = conn.execute(f"""
        ...
        GROUP BY subject ORDER BY cnt DESC LIMIT 50
    """, params).fetchall()
    ...
    top_domains = conn.execute(f"""
        ...
        GROUP BY sender_domain ORDER BY total DESC LIMIT 100
    """, params).fetchall()

    # Daily trend — no row cap; date filter already scopes the range
...
# ─────────────────────────────────────────────
EXPORT_COLUMNS = [
    "date", "sender_name", "sender_email", "sender_domain",
    "campaign", "journey", "phone_numbers",
...
EXPORT_HEADERS = [
    "Date", "From Name", "Email ID", "Domain",
    "Campaign", "Journey", "Phone Numbers",
...
def download_csv():
    ...
    rows = conn.execute(f"""
        SELECT e.date, e.sender_name, e.sender_email, e.sender_domain,
               e.campaign, e.journey, e.phone_numbers,
